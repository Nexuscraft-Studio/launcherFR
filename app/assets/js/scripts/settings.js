        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'Pour utiliser le Launcher, vous devez être connecté à au moins un compte. Vous devrez ensuite vous connecter à nouveau.<br><br>Êtes-vous sûr de vouloir vous déconnecter ?',
                    'Je suis sûr',
                    'Annuler'
                )
                setOverlayHandler(() => {
    ...
    authKeys.forEach((val) => {
        ...
        const accHtml = `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${acc.name}" src="https://nexuscraftrp.fun/api/skin-api/avatars/face/${acc.name}">
            </div>
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    ...
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">Rôle</div>
                        <div class="settingsAuthAccountDetailValue" style="color: ${acc.user_info.role.color};">${acc.user_info.role.name}</div>
                    </div>
                    <div class="settingsAuthAccountDetailPane">
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/Project-Zone-Delta/Zone_Delta_Launcher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `Nouvelle mises à jour disponible !`
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Télécharger le depuis GitHub<span style="font-size: 10px;color: gray;text-shadow: none !important;">Fermez le launcher est lancé le dmg pour lancer la mise à jour.</span>', false, () => {
        ...
        settingsUpdateTitle.innerHTML = 'Vous êtes en trains d\'utiliser la dernière version.'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Rechercher une mise à jour', false, () => {
            if(!isDev){
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Recherche en cours...', true)
            }
        })
    }
}
